...
where $\mathcal{L}$ is the population objective, $\theta^{(0)}$ the initialization, $\theta^{\star}$ its minimizer, and $\eta$ the step size, linking the compute budget $T(c)$ and data availability $N_{\text{eff}}(c,\delta)$ to the attainable excess risk at context $c$.
