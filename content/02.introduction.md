...
The formalism introduced by early works on time-varying and varying-coefficient network estimation provides a foothold [@doi:10.1073/pnas.0901910106; @doi:10.1093/bioinformatics/btp192; @kolar2009sparsistent]. Assume each observation $x_i$ is drawn from a distribution governed by parameters $\theta_i$:
$$
x_i \sim P(x;\,\theta_i).
$$
...
These functional forms provide new tools for understanding personalized and context-adaptive systems, but also present new modeling challenges. Estimating a unique $\theta_i$ from a single observation is ill-posed without structural regularization—smoothness, sparsity, shared representations, or latent grouping. And as adaptivity becomes more implicit (e.g., via neural networks or black-box inference), we need tools to recover, interpret, or constrain the underlying parameter variation.
...
\tag{★}
$$
where $\ell$ is a proper loss (e.g., squared, logistic), $S(c)\subseteq\{1,\dots,n\}\times\mathbb{N}$ is a **support set** selected for context $c$, and $\mathcal{R}(\theta;c)$ encodes how parameters are allowed to vary with context (smoothness, sparsity, low-rank, hierarchy, etc.). This decomposition into a context-dependent loss and a context-structured regularizer was first proposed in [@kolar2009sparsistent] and studied systematically in [@doi:10.1214/09-AOAS308]. These concepts remain two of the primary statistical tools for learning and understanding context-adaptive models.
