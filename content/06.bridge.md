...
directly from a sequence of in-context examples, where $X$ stacks the inputs, $y$ the outputs, $\lambda$ is the ridge penalty, and $I$ the identity. Each example $(x_i, y_i)$ is represented as a token, and the query token attends to the support tokens to compute the prediction for $x^\ast$; the attention mechanism learns to encode the solution to the regression problem [@doi:10.48550/arXiv.2211.15661].
...
(ii) joint features $\psi(x,c):=x\otimes \phi(c)\in\mathbb{R}^{d_x d_c}$ (with $\otimes$ the Kronecker product, and $d_x$, $d_c$ the dimensions of $x$ and $\phi(c)$),
